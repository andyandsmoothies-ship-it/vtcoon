...
- **Zero-Trust Plan Grilling**: Plans are flawed by default. Before user approval, invoke `plan-griller` (read-only) to audit physical disk and identify 1-3 concrete blind spots. Main agent verifies against physical files before updating `implementation_plan.md`. Zero blind compliance.
