...
- **Complexity Limits**: Áp dụng Khung Phân Loại 5 Tầng: Logic/FSM <= 400 LOC (ngưỡng cảnh báo tách module tại 300 LOC); UI Components <= 500 LOC; Tệp cấu hình tĩnh / Bảng tra cứu <= 800 LOC; Cyclomatic Complexity <= 5, tối đa 30 LOC/hàm. Tuyệt đối CẤM gộp dòng ("code golf"), xóa comment hoặc tạo hàm No-Op giả để né LOC; khi chạm ngưỡng bắt buộc tách module con.
- **Slice Scope Confinement**: Implement only the flows defined in the current ticket (Slice 1 implements Main Success Scenario only). Khi hoãn luồng Alternative (A#), BẮT BUỘC đăng ký vào Sổ Nợ Kỹ Thuật (Tech Debt Ledger) trong epic ledger kèm Slice đích tiếp nhận.
- **Test State Isolation**: Zero order-dependent tests. Run test suites with `--randomize`. Isolate test state và sử dụng Fixture/Schema Contract Tests đối chiếu 100% dữ liệu tĩnh với SSOT.
...
- **Subagent Artifact Persistence (Dual Output Pattern)**: Authoring subagents (planners, slicers) creating large artifacts (`docs/plans/`, `issues/`) MUST write directly to disk files using `write_to_file` in `Workspace: "inherit"` và return only a concise summary (<20 lines) with clickable file links. Read-only reviewers (`spec-reviewer`, `code-reviewer`) remain strictly read-only và report 1-page structured packets directly into chat.
...
1. Automated tests pass Adversarial Inversion (deliberate failure verification), carry traceability tags (`[UC-XXX/MSS]` or `[UC-XXX/A#]`), và bảo đảm Fixture Contract Test khớp 100% bảng SSOT.
...
3. Reviewer gates approve (`spec-reviewer` approves 100% Three-Way Spec Reconciliation against `docs/requirements.md` và kiểm tra nợ kỹ thuật mồ côi; `code-reviewer` approves code quality, architecture boundaries, và Lean Observability).
4. Progress is updated in `docs/epics/[epic]/_epic_ledger.md` (kèm Tech Debt Ledger nếu có hoãn flow) và user commits changes to git.
