...
/**
 * Thủ tục dự phòng: Tượng Ngựa Chiến Sapphire (Slot 3 - P4 Bé Bo)
 */
export function WarhorsePawnFallback({ config }: { readonly config: LuxuryPawnConfig }): React.ReactElement {
  return (
    <group position={[0, 0, 0]}>
...
/**
 * Component tập hợp điều phối Fallback thủ tục theo từng Slot
 */
export function LuxuryPawnProceduralFallback({
  ...
  return (
    <group position={[0, 0, 0]}>
      {safeIdx === 0 && <LandmarkTowerPawnFallback config={config} />}
      {safeIdx === 1 && <BayYachtPawnFallback config={config} />}
      {safeIdx === 2 && <ClassicCarPawnFallback config={config} />}
      {safeIdx === 3 && <WarhorsePawnFallback config={config} />}
    </group>
  );
}
