...
export function AuctionModal({
  ...
  timeRemaining,
  hasPassed = false,
  passedPlayerIds, declinedPlayerId,
  ...
  const effectivePassed = useMemo(() => new Set([...(passedPlayerIds ?? []), ...(hasPassed && myId ? [myId] : [])]), [passedPlayerIds, hasPassed, myId]);
  const competingCount = activePlayers.filter((p) => Boolean(p.id && !effectivePassed.has(p.id) && p.id !== declinedPlayerId)).length;
  ...
  return (
    ...
    >
      ...
      {/* Header BĐS Hợp Nhất (Hero Header & Điều khiển sàn đấu giá) */}
      <div data-testid="auction-hero-header" className="flex items-center justify-between gap-2 bg-amber-50/60 p-1.5 sm:p-2.5 rounded-2xl border border-amber-900/10 shrink-0">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <div className="w-3 h-8 sm:w-3.5 sm:h-9 rounded-md shrink-0 shadow border border-slate-900" style={{ backgroundColor: ribbonColor }} />
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {isConcluded ? (
            <span className="px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-extrabold bg-amber-100 text-amber-800 border border-amber-400 whitespace-nowrap">
              ĐÃ KẾT THÚC
            </span>
          ) : isForeclosure ? (
            <span className="px-1.5 sm:px-2.5 py-0.5 rounded-full text-[9px] sm:text-xs font-extrabold bg-rose-100 text-rose-800 border border-rose-400 flex items-center gap-0.5 sm:gap-1 animate-pulse whitespace-nowrap shrink-0">
              <span className="sm:hidden">⚠️ PHÁT MÃI</span>
              <span className="hidden sm:inline">⚠️ PHÁT MÃI CƯỠNG CHẾ (-30%)</span>
            </span>
          ) : null}
          {onClose && (
            <button
              ...
              className="min-w-[44px] min-h-[44px] w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-amber-100 hover:bg-amber-200 border border-amber-300 text-slate-800 font-black shadow-sm flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 transition-colors cursor-pointer shrink-0 text-sm"
            >
              ✕
            </button>
          )}
          ...
          <div className="w-full space-y-1 bg-amber-50/40 p-1.5 sm:p-2.5 rounded-xl sm:rounded-2xl border border-amber-900/10 order-3 md:order-2 md:col-start-2 md:row-start-2 md:pl-5">
            <div className="flex items-center justify-between text-xs pb-0.5 border-b border-amber-900/10">
              <span className="font-bold text-slate-700 uppercase tracking-wider text-[10px] sm:text-xs">
                ĐẠI GIA THAM GIA ({competingCount}/{activePlayers.length})
              </span>
            ...
            <div className="space-y-0.5 sm:space-y-1 max-h-16 sm:max-h-20 overflow-y-auto pr-1 scrollbar-none [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {activePlayers.length > 0 ? (
                activePlayers.map((p) => {
                  ...
                  const isPassed = !isBidder && Boolean(p.id && effectivePassed.has(p.id));
                  const isDebtorInForeclosure = Boolean(isForeclosure && (p.id === insolvencyPlayerId || (declinedPlayerId && p.id === declinedPlayerId)));
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-1.5 min-w-0">
                        ...
                        {isBidder ? (
                          <span className="text-[10px] font-black bg-amber-100 text-amber-700 px-1 py-0.2 rounded border border-amber-300 shrink-0">👑 Dẫn đầu</span>
                        ) : isPassed ? (
                          <span className="text-[10px] font-bold bg-slate-200 text-slate-600 px-1 py-0.2 rounded border border-slate-300 shrink-0">✕ Rút lui</span>
                        ) : isDebtorInForeclosure ? (
                          <span className="text-[10px] font-bold bg-rose-100 text-rose-700 px-1 py-0.2 rounded border border-rose-200 shrink-0">⚖️ Phát mãi</span>
                        ) : isDeclined ? (
                          <span className="text-[10px] font-bold bg-amber-100 text-amber-800 px-1 py-0.2 rounded border border-amber-300 shrink-0">🚫 Bỏ qua</span>
                        ) : null}
                      </div>
