    ...
    <div className={`p-3 rounded-xl flex flex-col space-y-2.5 ${matClass}`}>
      ...
      <div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-slate-500 font-bold shrink-0">Cộng nhanh:</span>
            <button
              type="button"
              data-testid="cash-stepper-increment"
              onClick={() => onCashChange(maxCash !== undefined ? Math.min(maxCash, cashVal + 100) : cashVal + 100)}
              className="flex-1 min-h-[44px] px-2.5 py-1.5 bg-slate-100 hover:bg-amber-100 text-slate-900 font-mono text-xs font-black rounded-lg border-2 border-slate-300 hover:border-amber-400 shadow-[0_2px_0_0_#cbd5e1] active:shadow-none active:translate-y-[2px] transition-all cursor-pointer inline-flex items-center justify-center"
