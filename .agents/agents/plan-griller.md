...
3. **The 5 Mandatory Stress-Test Pillars (5 Trục Phản Biện Bắt Buộc)**:
   - ⛓️ **Pillar 1: Data Origin-to-Sink Lifecycle (Vòng Đời Dữ Liệu Toàn Phần)**:
     ...
     - **Barter State Union Completeness**: In barter/exchange UI counters, tab badges, or validation logic, the active state must evaluate the union of assets and cash (`assets.length > 0 || cash > 0`), never asset-length only. Flag omissions as **[P1 - BARTER STATE TRUNCATION]**.
