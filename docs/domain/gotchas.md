...
### 261. [NET/UI/3D] Optimistic Roll Intent Guard & WebSocket Handshake Latency Phantom Dice Defense
- **Bẫy nghiệp vụ & kỹ thuật (Thực nghiệm iOS Chrome / Mobile Web)**:
  1. *Bẫy Kích Hoạt Lạc Quan Trước Kết Nối (Optimistic Premature Roll Trap)*: Hàm `handleRollDice` gọi `store.setIsRolling(true)` trước khi kiểm tra việc gửi intent lên máy chủ. Khi người chơi vừa vào ván hoặc máy chủ Render backend đang trong quá trình thức giấc/bắt tay TLS (`readyState === 0` CONNECTING), `sendIntent({ type: 'INTENT_ROLL' })` trả về `false` và âm thầm hủy gói tin.
  2. *Bẫy Xúc Xắc Ảo Lặp Lại 1+1 (Initial State Ghost Dice)*: Do máy chủ không nhận được lệnh, máy chủ không gieo xúc xắc ngẫu nhiên. Trong khi đó ở máy khách, hiệu ứng 3D `DiceTray` vẫn nhận cờ `isRolling: true` và hiển thị mặt xúc xắc theo giá trị mặc định trong bộ nhớ store (`INITIAL_GAME_STATE.dice: [1, 1]`). Kết quả là xúc xắc rơi ra 1+1 nhưng quân cờ đứng yên vì không có delta vị trí gửi về.
  3. *Bẫy Kẹt Trạng Thái Khóa Nút 1.5s*: `ActionDock` đặt cờ `isRollPending: true` khóa nút trong 1.5 giây. Nếu lệnh thất bại do mạng chưa mở, người chơi bị kẹt không thể bấm lại ngay lập tức.
- **Ràng buộc cứng & Thiết kế bất biến**:
...
### 273. [UI/CRAFT][UAT/TEST] Bất Biến Tinh Gọn Đơn Vị Tiền Tệ & Bẫy Khớp Chuỗi Tiêu Cực / Tiền Tố (Clean Currency Formatting & Substring Boundary Guard - IMP-197)
- **Bối cảnh & Bẫy thực tế**:
  1. *Bẫy Chuỗi Con Số Không Âm (`-0` Substring False Positive Trap)*: Khi kiểm tra triệt tiêu số không âm (`-0`) trên các component hiển thị số dư, nếu test suite dùng `.not.toContain('-0')`, mọi class Tailwind biên âm như `-bottom-0`, `-left-0`, `-top-0`, `-right-0` đều kích hoạt false positive làm rớt kiểm thử oan uổng.
  2. *Bẫy Tiền Tố Nút Thao Tác Nhanh (`+50` vs `+500` Collision Trap)*: Khi kiểm tra việc loại bỏ nút đấu giá nhỏ `+50`, nếu dùng `.not.toContain('+50')` thì nút bấm hợp lệ `+500` sẽ bị dính khớp chuỗi tiền tố và làm test fail.
  3. *Bẫy Nới Lỏng Assertions Khi Tiến Hóa Đặc Tả (Weakened Assertions Trap)*: Khi chuẩn hóa đơn vị tiền tệ loại bỏ `' Tr.'` trên toàn bộ hệ thống (`formatCurrency(12500) === '12.500'`), nếu lập trình viên lười biếng nới lỏng living tests thành `.not.toContain('Tr.')` hoặc xóa bỏ assertion thì hệ thống mất hoàn toàn lưới phòng thủ hồi quy số liệu.
- **Ràng buộc cứng & Thiết kế bất biến**:
  1. **Clean Number Currency SSOT Invariant**: Hàm `formatCurrency(val)` là nguồn chân lý duy nhất (SSOT) cho việc hiển thị tiền tệ, chỉ trả về chuỗi số phân tách dấu chấm gọn gàng (vd: `'12.500'`, `'-1.200'`, `'0'`), triệt tiêu 100% hậu tố `' Tr.'` hay `' Tr. VNĐ'` ở cấp độ giao diện và nhãn log.
  2. **HTML Boundary RegEx Testing Invariant**: Các phép kiểm tra giá trị số và nút bấm trong renderToStaticMarkup bắt buộc neo bằng biên thẻ HTML:
     - Chống `-0`: `expect(html).not.toMatch(/>\s*-\s*0\s*</)`.
     - Chống nhầm nút `+50`: `expect(html).not.toMatch(/>\+50</)`.
  3. **Rule 4 Literal Living Test Evolution**: Khi tiến hóa đặc tả (Specification Evolution), bắt buộc cập nhật các bài test kế thừa sang con số cụ thể thực tế (vd: `expect(html).toContain('1.500')`), bảo đảm 100% test sống sót mà không suy giảm độ nghiêm ngặt.
- **Traceability**: `[TC-197.01..20/MSS]`, `[UC-IMP197]`, `src/client/ui/ui_helpers.ts`, `src/client/3d/tile_texture_data.ts`, `tests/contracts/imp197_currency_unit_declutter.test.ts`.

---

### 274. [UI/CRAFT][FSM/RULE] Bất Biến Minh Bạch Người Tham Gia Đấu Giá & Bộ Đếm Cạnh Tranh Thời Gian Thực (Auction Participant Status Transparency & Competing Headcount Invariant - IMP-196)
- **Bối cảnh & Bẫy thực tế**:
  1. *Bẫy Rò Rỉ Trạng Thái Rút Lui Giữa Các Phiên Đấu Giá Khác Nhau (Cross-Auction Stale hasPassed Leak Trap)*: Khi một người chơi bấm Bỏ qua (`hasPassed: true`) ở phiên đấu giá đất A, nếu client chỉ tái sử dụng `prevPayload.hasPassed` mà không kiểm tra xem ô đất đang đấu giá có cùng phiên hay không (`delta.auction.cellIndex === prevPayload.cellIndex`), người chơi sẽ tự động bị đánh dấu là "Đã rút lui" ở phiên đấu giá đất B tiếp theo, mất quyền đấu thầu ngay từ đầu.
  2. *Bẫy Xén Cụt Huy Hiệu Trên Mobile 360px (Mobile 360px Header Badge Truncation Trap)*: Trong `AuctionModal`, tiêu đề thanh header khi có các badge dài như `⚠️ PHÁT MÃI CƯỠNG CHẾ (-30%)` kết hợp nút đóng `[✕]` trên màn hình hẹp 360x740 bị ép chiều rộng dẫn đến chữ bị xén cụt thành `"⚠️ PHÁT MÃI CƯỠNG..."`. Giải pháp: áp dụng Responsive Text Clamping với `<span className="sm:hidden">⚠️ PHÁT MÃI</span>` và `<span className="hidden sm:inline">⚠️ PHÁT MÃI CƯỠNG CHẾ (-30%)</span>`.
  3. *Bẫy Khớp Chuỗi RegEx Gợi Ý Giá Cược Khi Thêm Đơn Vị ' Tr.' (Chip Button Currency Suffix RegEx Collision Trap)*: Khi chuẩn hóa định dạng hiển thị tiền tệ, các nút chip gợi ý giá (`100% Gốc`, `120%`) trong `TradeModal` được kiểm tra bằng regex số thuần `\(\d+\)`. Nếu gắn `' Tr.'` vào các chip này sẽ làm gãy living tests. Trong khi đó trên `AuctionModal`, `HoseModal`, `TitleDeedModal`, nhãn tiền tệ yêu cầu hậu tố `' Tr.'` rõ ràng để bảo đảm độ xúc giác và tính dễ đọc.
  4. *Bẫy Tiêu Đề Rườm Rà Chiếm Diện Tích (Redundant Arena Title Trap)*: Tiêu đề "SÀN ĐẤU GIÁ TRỰC TUYẾN / LIVE TABLETOP ARENA" và huy hiệu "ĐANG MỞ" chiếm nguyên một hàng riêng biệt (~45px-50px) khiến màn hình di động hẹp (360x740) bị đẩy nội dung bục đấu thầu và danh sách người chơi ra ngoài viewport, buộc người chơi phải cuộn.
- **Ràng buộc cứng & Thiết kế bất biến**:
  1. **Auction Session Isolation & Guard Invariant**: Trong `apply_delta.ts`, kiểm tra `isSameAuction = prevPayload?.cellIndex === delta.auction.cellIndex && !prevPayload?.isConcluded`. Thuộc tính `hasPassed` chỉ được giữ nguyên nếu cùng một phiên đấu giá.
  2. **Auction Participant Transparency & Competing Headcount**: Tiêu đề danh sách đại gia bắt buộc hiển thị tỷ số cạnh tranh thực tế: `ĐẠI GIA THAM GIA ({competingCount}/{activePlayers.length})`. Người chơi rút lui được gắn huy hiệu `[✕ Rút lui]` và hiệu ứng `line-through` + `opacity-60`. Con nợ trong phát mãi cưỡng chế mang huy hiệu `[⚖️ Phát mãi]`, người bỏ qua ban đầu mang huy hiệu `[🚫 Bỏ qua]`. Người dẫn đầu luôn ưu tiên giữ huy hiệu `👑 Dẫn đầu`.
  3. **Responsive Header Badging Triad**: Các huy hiệu trạng thái nằm cạnh nút đóng `[✕]` trên header modal bắt buộc cung cấp nhãn rút gọn cho mobile hẹp (<640px) để không bao giờ bị xén cụt hoặc đẩy nút đóng khỏi viewport.
  4. **Unified Hero Header Invariant (Zero-Scroll Tabletop)**: Loại bỏ triệt để dòng tiêu đề thừa thãi và huy hiệu "ĐANG MỞ". Hợp nhất vạch màu BĐS, tên BĐS to rõ, subtitle giá khởi điểm, huy hiệu khẩn cấp (Phát mãi/Kết thúc nếu có) và nút đóng `[✕]` vào duy nhất một hàng Hero Header tinh gọn, giải phóng ~45px chiều cao vàng để 100% nội dung sàn đấu giá hiển thị trọn vẹn trong màn hình 360x740 không cần cuộn.
- **Traceability**: `[TC-196.01..23]`, `src/client/network/apply_delta.ts`, `src/client/ui/modals/modal_host.tsx`, `src/client/ui/modals/auction_modal.tsx`, `tests/client/imp196_auction_passed_players_transparency.test.ts`.

---

### 275. [3D/RENDER] Bất Biến Hiện Đại Hóa Sa Bàn 3D Mặt Đất, Giải Phóng Hero Landmark & Thanh Lọc Không Gian Bầu Trời (Ground Diorama Modernization & Clean Sky Invariant - IMP-197)
- **Bối cảnh & Bẫy thực tế**:
  1. *Bẫy Hạ Cào Bằng Chiều Cao Phá Vỡ Hợp Đồng (MaxHeight Contract Regression Trap)*: Khi giải phóng tháp Bitexco khỏi cảnh bị 10 cao ốc bao vây, nếu lập trình viên hạ cào bằng toàn bộ 10 tháp xuống 0.6 - 1.2m, test hợp đồng `TC-70.22` (`expect(maxHeight).toBeGreaterThanOrEqual(2.3)`) sẽ lập tức gãy. Giải pháp: Áp dụng quy hoạch giật cấp bậc thang thực tế — giữ 3 tháp hậu cảnh xa nhất (Z = -5.8 .. -5.9) ở mức 2.3m - 2.4m (nằm sâu phía sau không che khuất góc nhìn), và hạ mạnh 7 tháp cạnh sườn quanh Bitexco xuống 0.8m - 1.3m. Nhờ đó Bitexco (2.95m) vươn cao gấp >2.2 lần các khối bên cạnh, vừa tôn vinh ngọn tháp biểu tượng độc tôn, vừa bảo toàn 100% test hợp đồng cũ.
  2. *Bẫy Xung Đột Không Gian & Z-Fighting Móng Công Trình (Ghost Coordinates & Spatial Collision Trap)*: Khi bổ sung khối kiến trúc mới (khu căn hộ cao cấp có hồ bơi sân thượng), nếu không xác định tọa độ an toàn sẽ va chạm vào đường ray xe lửa (`|x|=6.9, |z|=6.9`) hoặc đại lộ xe chạy (`x=±4.65..4.95`). Ngoài ra, nếu đặt móng tại `y = 0.000` sẽ bị Z-fighting nhấp nháy với thảm cỏ `TERRAIN_BASE_Y = 0.000`. Giải pháp: Cố định tọa độ tại vùng cỏ Bán đảo Đông `[x: 3.2, y: 0.025, z: 0.5]` với móng `y = 0.025` (chuẩn `STANDEE_BASE_Y` trong `DEPTH_LAYER_STACK`).
  3. *Bẫy Hóa Vũng Đen Dạ Quang Ban Đêm (Nocturnal Shading Trap)*: Nếu mặt nước hồ bơi sân thượng và cửa sổ căn hộ dùng `meshStandardMaterial` đơn thuần mà không có `emissive`, khi chu kỳ môi trường chuyển sang `phase === 'night'`, hồ bơi sẽ biến thành một vũng dầu đen sì, triệt tiêu toàn bộ tính xúc giác Toybox. Giải pháp: Subscribe `useEnvironmentStore`, cấp `emissive="#0284C7"` và `emissiveIntensity={isNight ? 0.6 : isSunset ? 0.2 : 0.0}` cho hồ bơi, cùng `emissive="#FEF08A"` cho các ô cửa sổ ban đêm.
  4. *Bẫy Rác Bầu Trời & Tiến Hóa Đặc Tả (Clean Sky & Specification Evolution)*: Các khối mây lơ lửng và máy bay trên trời gây phân tán thị giác người chơi khỏi bàn cờ. Khi gỡ bỏ, các test cũ (như `coastal_island_environment.test.ts`) kiểm tra chuỗi `sphereGeometry` phải được tiến hóa đặc tả (Specification Evolution) sang kiểm tra bầu trời trong lành (`not.toContain('sphereGeometry')`) kết hợp lưới bảo tồn hạ tầng đại dương, tuyệt đối không chấp nhận test rớt do đặc tả tiến hóa.
- **Ràng buộc cứng & Thiết kế bất biến**:
  1. **Clean Sky Invariant**: Bầu trời 3D hoàn toàn sạch bóng các vật thể lơ lửng nhân tạo (máy bay) và cụm mây hình cầu, giữ nền trời pastel êm dịu để dồn 100% tiêu cự thị giác vào bàn cờ và sa bàn mặt đất.
  2. **Hero Landmark Stepped Prominence Invariant**: Tháp Bitexco (2.95m) giữ vị thế ngọn tháp cao nhất bàn cờ, cao gấp >= 2.0 lần so với chiều cao trung bình của các khối nhà văn phòng bao quanh.
  3. **Toybox Rooftop Tactility Invariant**: Khối chung cư cao cấp Bán đảo Đông tích hợp hồ bơi vô cực mini với nước xanh ngọc lam (#0EA5E9), viền cẩm thạch trắng (#F8FAFC), ghế tắm nắng, dù che và sân đỗ trực thăng Helipad `[H]` viền đỏ sắc nét, đạt chuẩn xúc giác đồ chơi cao cấp.
- **Traceability**: `[TC-197.01..28/MSS]`, `[UC-IMP197]`, `src/client/3d/coastal_island_environment.tsx`, `src/client/3d/horizon_mountain_range.tsx`, `src/client/3d/diorama/diorama_highrise_blocks.tsx`, `src/client/3d/diorama/diorama_residential_pool.tsx`, `src/client/3d/miniature_city_diorama.tsx`, `tests/contracts/imp197_diorama_ground_modernization.test.ts`.

---

### 276. [UI/CRAFT] Bất Biến Giải Phóng Bóp Nghẹt Container, Mở Khóa Chiều Cao Tự Nhiên & Chống Tụt Chữ Nút Bấm Đa Nền Tảng (Desktop ActionDock Text Drop Prevention & Responsive Button Layout Invariant - IMP-198)
- **Bối cảnh & Bẫy thực tế (Initial Deceptive Trap)**:
  1. *Bẫy Kế Thừa Chiều Cao Mobile Ép Khung Desktop (Mobile Height Lock Trap)*: Khi thiết kế nút bấm responsive, 4 nút phụ (`Quản Lý BĐS`, `Đàm Phán`, `Quy Hoạch`, `Hết Lượt`) được gán `h-11` (44px cố định) để đạt chuẩn WCAG tap-target trên mobile. Tuy nhiên khi lên desktop, nhãn chữ được bật lại (`hidden sm:inline`) kèm padding dọc `sm:py-2.5`, nhưng class `h-11` không bị ghi đè (thiếu `sm:h-auto`), khiến chiều cao nội dung vượt quá 44px và flex container đẩy nhãn chữ tụt xuống viền đáy hoặc biến mất.
  2. *Bẫy Bóp Nghẹt Chiều Rộng Container Cha (Ancestor Max-Width Bottleneck Trap)*: Trong `hud_container.tsx`, khối footer bọc ngoài ActionDock có class `sm:max-w-md` (448px). Khi ActionDock mở rộng trên desktop hiển thị đầy đủ cả 5 nút với nhãn chữ (~730px), container cha 448px bóp nghẹt các phần tử con, buộc trình duyệt bẻ dòng hoặc nén các nút phụ lại nếu thiếu `whitespace-nowrap` và `shrink-0`.
  3. *Bẫy Đè Lấn 183px Giữa Thông Báo Nổi & Thanh Giao Dịch Bot (Notice Chip vs Trade Strip Collision Trap)*: Khi bot gửi đề xuất giao dịch (`pendingTradeOffer`), `InlineBotTradeStrip` xuất hiện ngay phía trên dock. Nếu `actionDockNotice` (thông báo bot-pacing hoặc in-audit notice) vẫn render đồng thời, hai phần tử nổi đè lấn lên nhau 183px theo trục Y, che khuất tầm nhìn bàn cờ và các nút thao tác.
  4. *Bẫy Sai Chiều Điều Kiện Phủ Định (Inverted Selector Negation Trap)*: Trong `action_dock.tsx`, khi kiểm tra xem Bot Trade Strip có đang hiển thị cho người chơi hiện tại hay không, nếu viết nhầm `pendingTradeOffer.sellerId !== actingPlayerId` (thay vì `===`), chip thông báo sẽ bị ẩn sai thời điểm (khi người chơi là người mua/người ngoài) và hiển thị đè lấn đúng lúc người chơi nhận đề xuất.
- **Quan sát thực địa Scout (Scout Physical Finding)**:
  1. `action_dock.tsx` có thiết quân luật LOC nghiêm ngặt (hiện 397 LOC, trần 400 LOC được khóa bởi living test `imp193#L431`). Bổ sung selector `pendingTradeOffer` phải tuân thủ chuẩn 1-statement-per-line và chủ động prune đúng 1 dòng trống thừa để bảo toàn Delta LOC = 0.
  2. Ràng buộc `sm:max-w-md` cần được chuyển dịch vào đóng gói độc lập bên trong `InlineBotTradeStrip` (`bot_trade_offer_strip.tsx:134`), đồng thời khối `TelemetryBadge` cần bổ sung `shrink-0` để không bị co méo khi ActionDock mở rộng tự nhiên.
- **Ràng buộc cứng & Thiết kế bất biến (Verified Invariant)**:
  1. **Responsive Button Dual-Lock Invariant**: Mọi nút bấm sử dụng kích thước cố định trên mobile (`w-11 h-11`) khi mở rộng sang desktop bắt buộc phải đi kèm cả cặp giải phóng kích thước: `sm:w-auto sm:h-auto` kết hợp `shrink-0 whitespace-nowrap`.
